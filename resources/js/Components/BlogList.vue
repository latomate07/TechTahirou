                ...
                <div class="flex flex-wrap">
                    <div class="p-4 md:w-1/3" v-for="article in articles" :key="article.id">
                        <div class="h-full border-2 border-gray-200 dark:border-gray-800 border-opacity-60 rounded-lg overflow-hidden">
                            ...
                            <div class="p-6">
                                <h3 class="tracking-widest text-xs title-font font-medium text-gray-400 mb-1 uppercase">Categorie : {{ article.category.name }}</h3>
                                <h3 class="title-font text-lg font-medium text-gray-900 dark:text-white mb-3">{{ article.title }}</h3>
                                <p class="leading-relaxed dark:text-gray-300 mb-3">{{ article.content.slice(0, 90) + '...' }}</p>
                                <div class="flex items-center flex-wrap">
                                    <Link :href="route('blog.show', [article.slug])" class="text-yellow-500 inline-flex items-center md:mb-2 lg:mb-0">Voir plus
